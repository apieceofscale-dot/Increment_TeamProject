    ...
    {      
        ...
        for (int i = 1; i < lines.Length; i++) 
        {
            ...
            for (int col = 0; col < headers.Length; col++)
            {
                ...
                if (field == null)
                {
                    Debug.LogError($"{dataType.Name}?? [{headers[col]}] ??? ???????.");
                    return;
                    
                }
                //object value = Convert.ChangeType(values[col], field.FieldType); ?? ???? AnimationClip?? ?? ????.
