...
public class CharacterFacade : MonoBehaviour
{
    ...
    public event Action InventoryChanged // ?¥ê??? ?? ?????? ??? ????? ??????? ???? ???
    {
        add
        {
    ...
    public IReadOnlyList<CharacterInventoryEquipment> GetEquipmentInventory() // ???? ??? ?????? ?¬Ò? ???? ????
    {
    ...
    #region ??? UI ?? ??? ??????
    public int SkillSlotCount => CharacterControllers.SkillSlotCount; // 6??, ?¥å????? 0~5
    ...
    #region ?????? ?? ?¥ê??? ??
    public bool AddEquipment(ItemStatus status, out Guid instanceId) // ??? 1?? ???, ???? ???¥ï? ???? ID ???
    {
    ...
    public IReadOnlyList<CharacterInventoryEquipment> GetInventoryEquipment() // ?¬Ò? ???? ??? ????
    {
