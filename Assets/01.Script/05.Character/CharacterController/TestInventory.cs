...
public class TestInventory : MonoBehaviour
{
    ...
    private void AddTestArmor()
    {
        ...
        if (success) lastTestEquipmentId = instanceId;
        source.Clear();
    ...
    private void RemoveTestArmor()
    {
        ...
        if (success) lastTestEquipmentId = Guid.Empty;
    ...
    private void CheckArmorRules()
    {
        ...
        try
        {
            ...
            Check(!first.TryAddEquipment(null, out Guid rejected) && rejected == Guid.Empty,
                "null 입력 거부");

            ItemStatus source = new ItemStatus();
            source.Reset(100, ItemType.Weapon, 10, 1, 0, 0, 10);
            Check(!first.TryAddEquipment(source, out rejected) && rejected == Guid.Empty,
                "무기 거부");
            ...
            Check(source.BaseValue == 10 && source.EffectiveValue == 99 && !source.PickedUp,
                "공유 상태를 변경하지 않음");
            Check(first.TryAddEquipment(source, out Guid secondId) && firstId != secondId,
                "같은 종류도 개별 보관");
            source.Clear();
            Check(first.TryGetEquipment(firstId, out CharacterInventoryEquipment saved) &&
                saved.BaseValue == 10 && saved.ItemId == 100, "원본 초기화 후 기본값 보존");
            Check(first.GetEquipmentCount(100) == 2 && second.EquipmentCount == 0,
                "종류별 개수와 캐릭터별 독립");
            firstObject.SetActive(false);
            firstObject.SetActive(true);
            Check(first.EquipmentCount == 2, "재활성화 후 보유량 유지");
            var snapshot = first.GetEquipmentSnapshot();
            Check(first.TryRemoveEquipment(firstId) && first.TryGetEquipment(secondId, out _),
                "선택한 개별 ID만 제거");
